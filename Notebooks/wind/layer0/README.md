...
Plan-of-record: [`docs/plans/wind/00_hazard_definition.md`](../../../docs/plans/wind/00_hazard_definition.md).
...
- **Severity form** = a **bounded GPD** (ξ<0) on gust, anchored at μ, truncated at L — the reusable analytic solve
  (`ξ=(μ_mean−μ)/(μ_mean−L)`, `σ=−ξ(L−μ)`), **verified against known answers** (mean==μ_mean, support==[μ,L]).
  μ_mean is **fit to observed gusts in M1**, *not* back-solved from an EAL (the old repo's rejected habit).
- **Strong-wind tail route** = the **ASCE 7-22 RP surface** as a pre-computed return-level curve (profile-assembly).
- A **coupling-taxonomy primer** — tornado (areal) · strong wind (site-conditioned) · hurricane (field-intensity,
  deferred); one shared engine behind all three.
...
Decisions [DD-WN-6/7/8/11](../../../docs/plans/wind/decisions.md) (observable; two thresholds; bound L; anchored
...
[`assumptions.md`](../../../docs/plans/wind/assumptions.md).
