...
#
# > Plan-of-record: [`00_hazard_definition.md`](../../../docs/plans/wind/00_hazard_definition.md) · Reasoning:
# > [`discussion/wind/03`](../../../docs/extra/discussion/wind/03_hazard_definition_and_thresholds.md) (standards)
# > + [`discussion/wind/02`](../../../docs/extra/discussion/wind/02_coupling_buckets_and_wind.md) (coupling) ·
# > Decisions [`DD-WN-*`](../../../docs/plans/wind/decisions.md) · Assumptions [`AWN-*`](../../../docs/plans/wind/assumptions.md).
...
# **damage** — this tier. Same medium, different question — [discussion/01 §7](../../../docs/extra/discussion/wind/01_scope_and_sub_peril_taxonomy.md).)
#
...
# is needed (an M2 step, [AWN-15](../../../docs/plans/wind/assumptions.md)).
#
...
# `ξ>0` heavy/unbounded, `ξ=0` exponential, **`ξ<0` bounded** (a finite upper endpoint). A physical ceiling L
# forces **ξ < 0** — a *short* tail. We reuse the old repo's closed-form solve (left-anchor μ, right-bound L,
# mean-constrained), and **verify it against known answers** (basics-spot-on):
...
# Illustrative strong-wind values (m/s): μ = NWS severe; μ_mean = mean event gust (most events barely severe,
# so just above μ — a DEFINITIONAL placeholder; M1 fits it to the real gust record, AWN-18). L = EF5 ceiling.
mu_ms, mu_mean_ms = ms(58.0), 31.0          # 25.9 m/s, 31 m/s
...
ax.plot(v, exp_dist.pdf(v), lw=1.2, ls="--", color="gray", label="exponential (ξ=0, unbounded) — for contrast")
...
plt.tight_layout(); plt.show()
print("Takeaway: the bounded GPD (ξ<0) puts ZERO mass beyond L — the tail is short and physical, unlike an")
print("unbounded exponential. This is the FORM the catalog will carry; M1 fits μ_mean to real gusts (not an EAL).")
...
# (*point / narrow path / broad swath / regional field*). Wind's three sub-perils land in **all three** — the
# "sub-perils matter" payoff.

# %%
buckets = pd.DataFrame([
    ("1. Areal hit-or-miss", "point / narrow path", "full loss IF hit, $0 if missed (bimodal)",
     "Bernoulli × Minkowski (√F+√s)²/A", "TORNADO", "hail ✅ (reuse)"),
    ("2. Field-intensity", "regional field", "a local value, different every event",
     "sample the field per event", "HURRICANE (deferred)", "— none yet"),
    ("3. Site-conditioned", "broad swath (pre-integrated)", "its own pre-baked local profile (no miss)",
     "profile-assembly (read it)", "STRONG WIND", "wildfire ✅ (reuse)"),
], columns=["bucket", "footprint", "what the asset reads", "math", "wind sub-peril", "built?"])
...
print("HURRICANE → bucket 2 (regional field; the genuinely UNBUILT bucket — portfolio correlation + EVT load-")
print("  bearing; named, fenced, DEFERRED). One shared compound-Poisson/NegBin engine, untouched, behind all three.")
