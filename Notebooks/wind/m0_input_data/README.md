...
**compared**. Plan: [`docs/plans/wind/m0_input_data.md`](../../../docs/plans/wind/m0_input_data.md) ·
...
profile, no fit). **Wind is the first peril that needs both**, because its sub-perils land in different coupling
buckets ([discussion/wind/02](../../../docs/extra/discussion/wind/02_coupling_buckets_and_wind.md)):
...
Both feed the same M1, which **forks per sub-peril** ([DD-WN-3](../../../docs/plans/wind/decisions.md) strong
wind, [DD-WN-5](../../../docs/plans/wind/decisions.md) tornado).
...
`data/wind/<slug>_wind_m0_asce.parquet` (RP→gust table; gitignored) + `…_manifest.json` (kept). Raw `identify`
JSON cached under `data/wind/raw/<slug>/` (gitignored).

## Key decisions & assumptions (this layer)

[DD-WN-3](../../../docs/plans/wind/decisions.md) (ASCE surface = profile-assembly, no Hydronos) ·
[DD-WN-6/7](../../../docs/plans/wind/decisions.md) (3-s gust; two thresholds). Assumptions **AWN-5** (3-s gust),
...
in `02`. Full register: [`assumptions.md`](../../../docs/plans/wind/assumptions.md).
