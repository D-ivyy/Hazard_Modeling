...
→ [M4 loss](m4_loss_metrics.md). Built for both wind farms. Notebook: `Notebooks/wind/m3_damage/01_damage`.
...
the bounded GPD) → a **conditional damage ratio** → `conditional_loss = Asset_DR(v) × exposure × TIV`. For tornado,
...
`data/wind/<asset>_wind_m3_damage_strongwind.parquet` + `…_tornado.parquet` (conditional DR + loss vs gust) +
