...
**How it works (the exploration):** [discussion/wind/02](../../extra/discussion/wind/02_coupling_buckets_and_wind.md).
Notebooks (M2 forks — two coupling buckets): `Notebooks/wind/strong_wind/m2_coupling/01_coupling` (site-conditioned)
and `Notebooks/wind/tornado/m2_coupling/01_coupling` (areal).
...
[discussion/wind/02](../../extra/discussion/wind/02_coupling_buckets_and_wind.md)). The teaching question is *"how
...
| Bucket | Physics (footprint) | What the asset reads | Math | Built reference peril | Wind sub-peril |
|---|---|---|---|---|---|
| **1. Areal hit-or-miss** | finite footprint **covers you or misses you** (binary) — *narrow path* | full conditional loss on a hit, **$0** on a miss | Bernoulli hit × **Minkowski** `p = (√F+√s)²/A`; `λ_asset = λ_collection · p` | **hail** ✅ | **tornado** |
| **2. Field-intensity** | one event = a **continuous intensity field** over space — *regional field* | the field value **at your location**, varying event-to-event | sample-and-weight the field per event | hurricane (unbuilt) | hurricane — **DEFERRED** |
| **3. Site-conditioned** | a **pre-integrated per-site profile** (frequency + intensity already aggregated) — *broad swath* | **your own local profile** — no hit-or-miss | read the profile; **no spatial factor** | **wildfire** ✅ | **strong / straight-line wind** |
...
>
> **Wind's payoff:** its sub-perils span **two already-built buckets** — tornado reuses hail's areal coupling,
> strong wind reuses wildfire's site-conditioned coupling — leaving only field-intensity (hurricane) for later.
> Inland-convective wind tours the two we have built; it builds **no new coupling physics**, only new
...
To make the taxonomy crystal-clear, M2 names what wind is **not** building yet. **Hurricane** is bucket 2,
...
curve, `x₀ ≈ 160 mph`). → [00_intent](00_intent.md) deferrals, [discussion/wind/02](../../extra/discussion/wind/02_coupling_buckets_and_wind.md).
...
**Strong wind (site-conditioned):** `{ lambda_per_yr (from M1), severity_distribution (3-s gust, bounded GPD),
exposure_fraction = 1.0, p_hit = 1.0, portfolio_correlation = "documented, deferred", tiv, footprint }` →
occurrence × conditional gust × whole-farm exposure, **no spatial factor**.
...
multi-asset / field-intensity build) · **field-intensity coupling (hurricane, bucket 2)** — Holland wind field →
swath grid, the genuinely unbuilt bucket · explicit terrain-exposure adjustment of the ASCE surface (B/D vs C). All
in [`assumptions.md`](assumptions.md) and [discussion/wind/02](../../extra/discussion/wind/02_coupling_buckets_and_wind.md).
