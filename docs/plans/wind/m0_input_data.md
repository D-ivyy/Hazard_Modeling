...
[00_hazard_definition](00_hazard_definition.md). **Why these sources:** [discussion/wind/03](../../extra/discussion/wind/03_hazard_definition_and_thresholds.md).
...
**Wind is the first peril that needs both**, because its sub-perils land in different coupling buckets
...
USWTDB rows, boundary polygons) to `data/wind/<asset>_raw/` (**gitignored**; keep a small manifest).
...
3. **Field-dictionary every value** (value + meaning + reference base + units): basic wind speed = **3-s peak gust
   ...
   risk-category→MRI map (AWN-15, surface vintage = ASCE 7-22). Cite [data dictionary 03](../../extra/discussion/wind/03_hazard_definition_and_thresholds.md).
...
7. **Emit** `data/wind/<asset>_wind_m0_asce.parquet` (RP→gust table per site, per sub-peril) + a kept JSON manifest
...
7. **Emit** `data/wind/<asset>_wind_m0_spc.parquet` (de-duplicated tornado + convective-wind events, path stats) +
...
3. **TIV** — per-turbine value × turbine count (or $/kW × MW); subsystem cost-split deferred to [M3](m3_damage.md).
   ...
   as **% of TIV alongside $**.
4. **Emit** `data/wind/<asset>_wind_m0_geometry.parquet` (turbine points + boundary polygon + TIV) + manifest.
...
```text
data/wind/<asset>_raw/                          raw ASCE JSON, SPC/Storm-Events, USWTDB, polygon   (gitignored)
data/wind/<asset>_wind_m0_asce.parquet          ASCE RP→gust table (strong wind + Ch 32 tornado)   (gitignored)
data/wind/<asset>_wind_m0_spc.parquet           de-duplicated SPC tornado + convective-wind events (gitignored)
data/wind/<asset>_wind_m0_geometry.parquet      USWTDB points + boundary polygon + TIV             (gitignored)
data/wind/<asset>_wind_m0_*_manifest.json       provenance + units + bias-correction TODO          (kept)
```
...
2. **Source & provenance** — pin the exact **ASCE edition (7-22)**, the **SVRGIS / Storm-Events windows + radius**,
   the **USWTDB vintage**; cache the raw bytes under `data/wind/<asset>_raw/`.
...
9. **Cross-references** — link every concept to [00_hazard_definition](00_hazard_definition.md),
   [discussion/wind/02](../../extra/discussion/wind/02_coupling_buckets_and_wind.md),
   [discussion/wind/03](../../extra/discussion/wind/03_hazard_definition_and_thresholds.md), the
...
create `Notebooks/wind/m0_input_data/` and build `01_asce_hazard` first (the pre-integrated surface — fastest path
...
(profile-assembly), tornado from the bias-corrected SPC fit (`λ` + bounded-GPD/EF severity).
