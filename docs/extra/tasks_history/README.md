...
|---|---|---|---|
| 2026-06-17 | flood-solar | [`m0-m4-eal-hardened`](2026-06-17__flood-solar__m0-m4-eal-hardened/handoff.md) | Flood (riverine) × solar built **M0→M4** on real public data (FEMA BLE depth + canonical RIVERINE_FLOOD×solar curves + annual-max MC); sites Elizabeth LA (proving) + Hayhurst TX (baseline). This session = **EAL hardening ([JD-FL-8](../../plans/flood/decisions.md))**: densified the frequent floods (10/25/50-yr) via USGS NLDI→NSS regression flow-frequency + a **BLE-anchored power-law rating** (replacing the assumed 0.5 ft onset); 10-yr depth 0.5→**0.97 ft**, EAL **0.13→0.155% TIV**, PML@100/500 unchanged; depths **slope-invariant** (anchors cancel the weak input). + learning log 11. Next = regression-Q uncertainty → distributional EAL; then flood × wind-farm. |
