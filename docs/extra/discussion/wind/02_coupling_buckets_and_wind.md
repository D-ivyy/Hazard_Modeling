...
because its three sub-perils sit in **three different coupling buckets** — so by the end you will see the
whole platform's coupling map, and "field-intensity vs the rest" will finally click. A discussion doc, not a
plan; it settles *how the coupling works* before the M2 plans (`docs/plans/wind/m2_coupling.md`).*
...
4. **Wind's three sub-perils land in all three buckets** — that is the "sub-perils matter" payoff. Inland-
   convective wind (tornado + strong wind) tours **two already-built buckets** (hail's areal, wildfire's
   site-conditioned); only hurricane needs the third (field-intensity), which is why it is deferred.
...
to split the wind *family* in [`01`](01_scope_and_sub_peril_taxonomy.md): does the peril have a distinct
**footprint geometry** and a distinct **data/magnitude metric**? Footprint is the part that drives coupling.
...
Field-intensity is the only bucket we have **not** built — which is exactly why hurricane (the field-intensity
member of the wind family) is deferred.
...
## 6. Where each wind sub-peril sits — and *why*

Now the payoff. Wind's three sub-perils land in three different buckets — this is *why* "sub-perils matter"
is not a slogan.
...
### Hurricane → Bucket 2 (field-intensity), DEFERRED

A hurricane is the reference's **regional field** footprint, and it is scored buildable on every axis (public
...
```text
  ...
  WIND SUB-PERILS        TORNADO                   STRONG WIND                HURRICANE (deferred)
  other examples         fire perimeter           flood depth-RP grids       earthquake, snow, ice
```

Read the bottom three rows: **hail proved bucket 1, wildfire proved bucket 3, and bucket 2 is empty.**
Inland-convective wind reuses the two proven buckets (tornado → bucket 1, strong wind → bucket 3) and **names
hurricane as the build that finally fills bucket 2.** That is the elegance of doing wind third — it tours the
platform's whole coupling map, and it tells us *exactly* what is left unbuilt and where the new machinery
(field generation + portfolio EVT) will have to go.
...
> (reuse wildfire's M2; ASCE RP surface as pre-integrated hazard)** · **hurricane = field-intensity, deferred.**
> The shared compound-Poisson/NegBin engine is **untouched** in all three. *(Recommended: yes — reference
> §footprint-taxonomy + the [W]/[T] correlation note + the old repo's mis-coupling self-audit all converge.)*

---

*Next: turn the committed assignments into `docs/plans/wind/m2_coupling.md` (the plan-of-record — the
