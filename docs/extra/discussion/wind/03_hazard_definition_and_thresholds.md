...
**two-threshold** insight, EVT-on-magnitude / the bounded GPD, and the ASCE return-period surface as a
pre-computed return-level curve. It is written for a reader new to the domain, so each term is defined on
first use. The conclusions graduate to [`docs/plans/wind/00_hazard_definition.md`](../../../plans/wind/00_hazard_definition.md)
and [`decisions.md`](../../../plans/wind/decisions.md).*
...
4. **The physical upper bound** L = the **EF5 ceiling ≈ 113 m/s (~253 mph)** — gust magnitude is bounded; the
   severity tail is *short*, not heavy.
5. **Severity = a bounded Generalized Pareto Distribution (GPD) on gust magnitude above μ, truncated at L** —
   EVT-on-intensity, the reusable analytic solve from the old repo's `mag_sim`. Continuous magnitude (unlike
   wildfire's 6 discrete flame-length classes) — so wind finally cashes in the continuous-severity thread.
...
essentially no probability mass. We anchor **L = the EF5 ceiling ≈ 113 m/s (~253 mph)** (the old repo's
HAZARD_LIMITS value; note the old repo's F-scale midpoints put F5 at 111.8 m/s — consistent). This bound is
what makes the severity tail **short** (§5).
...
> 🔧 **Divergence to reconcile (from prior art).** The old repo's tornado limit was **145 m/s** (Moore-1999
> *observed Doppler max*), while it placed **113 m/s** as the *Strong Wind* limit (Cyclone Olivia gust). The
> new build adopts the **EF5 damage-inferred ceiling ≈ 113 m/s** as L for the convective members. Record the
> reconciliation (damage-inferred ceiling vs observed Doppler max) as a decision, not a silent change.

---

## 5. Severity — a bounded GPD on gust magnitude (EVT-on-intensity)

Now the severity model: *given* an event above μ, how big is the gust? The answer is **Extreme Value Theory
on the intensity** — fit the tail of the gust magnitude, then map the rare gust through the damage curve.
...
```text
  ξ > 0   heavy / unbounded tail        (catastrophe-loss intuition — but NOT right here)
  ξ = 0   exponential tail
  ξ < 0   BOUNDED, finite upper endpoint ← THIS is wind: a physical ceiling L exists
```
...
bounded.* Wind's gust is bounded at L (EF5 ceiling), so **ξ < 0** — a *short* upper tail. The old repo's
`mag_sim` already has the closed-form solve we reuse (left-anchored at μ, right-bounded at L, mean-constrained):

```text
  ξ = (μ_mean − μ) / (μ_mean − L)        ← μ_mean − L < 0  ⇒  ξ < 0  ⇒  bounded tail
  σ = −ξ · (L − μ)
...
> **DECISION WN.** Severity = **bounded GPD (ξ<0) on the 3-s gust, anchored at μ, truncated at L**, fit to the
> SPC/Storm-Events record (bias-corrected) — *not* EAL-anchored? *(Recommended: yes — EVT-on-intensity per
> hazard_math/05; reuse the old analytic solve, reject the EAL back-solve.)*
...
([`docs/plans/wind/00_hazard_definition.md`](../../../plans/wind/00_hazard_definition.md)):
...
```text
  ...
  Physical bound L     :  EF5 ceiling ≈ 113 m/s (~253 mph)     [reconcile vs old-repo 145 m/s Doppler max]
  Damage-onset (2nd)   :  IEC 61400 survival/design — Ve50 ≈ 1.4·Vref ≈ 52–70 m/s  [curve leaves zero here]
  Severity model       :  bounded GPD (ξ<0) on the 3-s gust, anchored μ, truncated L
                          — fit to SPC/Storm-Events (bias-corrected)  OR
                          — read the ASCE RP surface (pre-computed return-level curve, strong wind)
  Operational state    :  feathered vs operating affects the curve (per the reference) — registered for M3
```

---

*Next: these graduate to `docs/plans/wind/00_hazard_definition.md` (the authored layer-0 spec) and the
